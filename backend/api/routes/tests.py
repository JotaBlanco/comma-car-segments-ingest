...
def list_tests(
    ...
    query = query_params.model_dump(
        exclude_none=True, exclude={"q", "device_id", "has_vmodel", "page", "page_size"}
    )
    if "test_id" in query:
        query["_id"] = query.pop("test_id")

    # A test carrying a vmodel sub-document is a V-model Test Run; one without it is a
    # Phase 2 bench execution record. Unset by default, so the list is unchanged.
    if query_params.has_vmodel is not None:
        query["vmodel"] = {"$ne": None} if query_params.has_vmodel else None
