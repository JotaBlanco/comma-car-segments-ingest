...
@canvas.notebook(position=(1004, 2861), size=(1010, 719), code_height=200, viz={'outputCell': 0})
...
def ai_1():
    ...
    Use can_signals_sample and maybe signal_catalog too as source of data for a full analysis. Create one notebook notebook cell per domain signal group (braking, steering, navigation, etc.) and one above those where understanding each signal, their domain, how they interact, the hierarchy relationships, which upper hierarchy they belong to, what they mean, some examples of data visualisation, etc. With what you gave me I still don't know anythign about the data. Have a domain knowledge angle too. It's over 1000 signals, so think well how to structure the visualisation"""
