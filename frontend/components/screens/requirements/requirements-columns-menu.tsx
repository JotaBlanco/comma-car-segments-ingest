...
import { setHiddenColumns } from "./requirements-column-visibility";
import { REQUIREMENT_COLUMNS } from "./requirements-columns";

/**
 * Which columns the requirements grid draws. The choice is this browser's
 * (see `requirements-column-visibility.ts`) and it changes the TABLE only —
 * the export writes every column whatever is ticked here.
 *
 * A pinned column shows its tick disabled rather than vanishing from the
 * list, so a person hunting for "Requirement" finds it and learns why it
 * cannot go.
 */
export function RequirementsColumnsMenu({ hidden }: { readonly hidden: readonly string[] }) {
  const hiddenSet = new Set(hidden);
  const shown = REQUIREMENT_COLUMNS.filter(
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        aria-label={`Columns — ${shown} of ${REQUIREMENT_COLUMNS.length} shown`}
      ...
      >
        ...
        {hidden.length > 0 && (
          <span className="ml-px font-mono text-[0.68rem] text-ink-3">
            {`${shown}/${REQUIREMENT_COLUMNS.length}`}
          </span>
        )}
      </DropdownMenuTrigger>
